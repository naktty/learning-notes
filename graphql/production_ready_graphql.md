...
# GraphQL API ドキュメンテーション
* GraphQLドキュメンテーション・ジェネレーターはAPIリファレンスには最適だが、人間味に欠ける。
* ドキュメンテーションは、APIで何が可能なのかに焦点を当てるべきである。
* Changelogと今後の変更ページは、クライアントがAPIの進化に追いつくための素晴らしい方法だ。

# 他のAPIスタイルからの移行
* GraphQLとRESTの両方を移行または維持するための重要ポイント
  * 新しいGraphQL APIをREST APIのGraphQLバージョンとして見るのではなく、ドメインとユースケースに対する新しいGraphQLインターフェイス
  として見ること。
  * REST上のGraphQLは良いパターンだが、N+1問題やその他のパフォーマンス問題に注意すること。
  * RESTエンドポイントをGraphQLにコピーするのではなく、ユースケースを考え、それを解決するためにGraphQL APIを設計すること
  * もしコードの再利用が問題なら、REST APIやGraphQL APIにロジックを集中させないようにし、このロジックのために別のレイヤーを探すようにする。この解決策として、ドメイン駆動設計を見てみることをお勧めする。
